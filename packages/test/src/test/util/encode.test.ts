...
describe("imageBinaryToBlob", () => {
  ...
  test("default mime emits PNG bytes with PNG type", async () => {
    const blob = await imageBinaryToBlob(bin);
  ...
  test("explicit image/jpeg emits JPEG bytes with JPEG type", async () => {
    ...
    const dataUri = await imageBinaryToDataUri(bin, "image/jpeg");
    expect(dataUri.startsWith("data:image/jpeg;")).toBe(true);

    const blob = await imageBinaryToBlob(bin, "image/jpeg");
