...
async function getFilesetResolverClass(): Promise<
  typeof import("@mediapipe/tasks-text").FilesetResolver
> {
  const mod = await import("@mediapipe/tasks-text");
  return mod.FilesetResolver;
}

async function getVisionFilesetResolverClass(): Promise<
  typeof import("@mediapipe/tasks-vision").FilesetResolver
> {
...
const getWasmTask = async (
  ...
  switch (task_engine) {
    case "vision": {
      const FilesetResolver = await getVisionFilesetResolverClass();
    ...
    case "text": {
      const FilesetResolver = await getFilesetResolverClass();
