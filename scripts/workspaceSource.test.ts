...
import { SOURCE_STUB_SENTINEL, stubSpecsFor } from "./lib/sourceStubs";
...
import {
  ...
  ownerOf,
  resolveTestTarget,
  resolveWorkspaceDistId,
  resolveWorkspaceSourceId,
  stubbedBundleMessage,
  TEST_TARGETS,
...
/**
 * A throwaway `dist` holding one `use-source` stub and one real-looking bundle.
 * Written rather than pointed at the tree, so the probe is driven from both
 * sides whichever mode the checkout happens to be in.
 */
const stubDir = mkdtempSync(join(tmpdir(), "workglow-stub-probe-"));
writeFileSync(
  join(stubDir, "node.js"),
  `// ${SOURCE_STUB_SENTINEL}: generated by \`use-source\`.\nexport * from "../src/node.ts";\n`
);
writeFileSync(join(stubDir, "bundle.js"), "// src/index.ts\nexport const x = 1;\n");
...
describe("workspace source resolution", () => {
  ...
  describe("resolveId", () => {
    ...
    /**
     * The `dist` target's arm. `use-source` (or `rebuild`) is the normal state
     * of a tree here and `use-dist` is rarely run, so the tree this target is
     * reached for usually has stubs in `dist` — where every bundle-shaped
     * assertion passes for the wrong reason and the run reports a clean sweep
     * over bundles it never loaded.
     */
    describe("dist target", () => {
      ...
      it("refuses a resolution that landed on a use-source stub", async () => {
        const stub = join(stubDir, "node.js");
        const { context } = recordingContext({ id: stub });

        await expect(
          resolveWorkspaceDistId(packages, context, "@workglow/ai", undefined, {})
        ).rejects.toThrow(/is a `use-source` stub re-exporting src/);
      });

      it("passes a non-workspace specifier through without resolving it", async () => {
        const { calls, context } = recordingContext({ id: "/elsewhere/vitest.js" });

        expect(await resolveWorkspaceDistId(packages, context, "vitest", undefined, {})).toBeNull();
        expect(calls).toEqual([]);
      });

      it("reports an unresolvable specifier the same way the source arm does", async () => {
        const { context } = recordingContext(null);

        await expect(
          resolveWorkspaceDistId(packages, context, "@workglow/ai/not-exported", undefined, {})
        ).rejects.toThrow(/cannot resolve "@workglow\/ai\/not-exported"/);
      });
    });
  });

  describe("stub probe", () => {
    it("recognises what use-source writes, and nothing else", () => {
      expect(isSourceStubFile(join(stubDir, "node.js"))).toBe(true);
      expect(isSourceStubFile(join(stubDir, "bundle.js"))).toBe(false);
    });

    it("answers false for a file it cannot read", () => {
      // The import fails a moment later with a better message than this could
      // give, so an unreadable entry is not worth a second error of its own.
      expect(isSourceStubFile(join(stubDir, "absent.js"))).toBe(false);
    });
  });
  ...
  describe("plugin attachment", () => {
    ...
    const carriesGuard = (project: ConfiguredProject): boolean =>
      (project.plugins ?? []).some((plugin) => plugin?.name === DIST_BUNDLE_GUARD_PLUGIN_NAME);
    ...
    it("swaps in the dist guard, and only it, when the target is dist", async () => {
      const projects = await projectsForTarget("dist");
      expect(projects.filter(carriesPlugin)).toEqual([]);
      expect(projects.filter((project) => !carriesGuard(project))).toEqual([]);
    });

    it("attaches the guard to no project under the default target", async () => {
      expect((await projectsForTarget("source")).filter(carriesGuard)).toEqual([]);
    });
    ...
    it("names the plugins the same thing their diagnostics do", () => {
      expect(workspaceSourcePlugin(ROOT, packages).name).toBe(WORKSPACE_SOURCE_PLUGIN_NAME);
      expect(distBundleGuardPlugin(ROOT, packages).name).toBe(DIST_BUNDLE_GUARD_PLUGIN_NAME);
      expect(stubbedBundleMessage("/repo/packages/ai/dist/node.js")).toContain(
        `[${DIST_BUNDLE_GUARD_PLUGIN_NAME}]`
      );
